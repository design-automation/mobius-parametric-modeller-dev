 ...
 */
export class AttribsGetSet extends AttribsMerge {
    /**
     * Constructor
     */
    ...
    private _setAttribs(new_attribs_data: IAttribData[], ent_type: EEntType) {
        const to_attribs: Map<string, AttribMap> = new Map();
        new_attribs_data.forEach( new_attrib_data => {
            const name: string = new_attrib_data.name;
            // create a new attrib
            const to_attrib: AttribMap = new AttribMap( name, new_attrib_data.data_type );

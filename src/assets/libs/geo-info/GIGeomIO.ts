import { TTri, TVert, TEdge, TWire, TFace,
    TColl, IGeomData, TPoint, TPline, TPgon, IGeomArrays, IGeomPack, TPosi, IIDArrays, IIDMaps, TId } from './common';
...
export class GIGeomIO {
    ...
    //         const new_posi_i: number = posi_i + num_posis;
    //         posis_map.set(posi_i, new_posi_i);
    //     }
    //     // vertices
    ...
    public mergeAndPurge(geom_arrays: IGeomArrays): Map<number, number>[] {
        console.log("============CALLING MERGE AND PURGE==================");
        // console.log("THIS COUNTS", JSON.stringify(this._geom_arrays.id_counts));
        // console.log("THIS MAPS", this._geom_arrays.id_maps);
        // console.log("INCOMING COUNTS", JSON.stringify(geom_arrays.id_counts));
        // console.log("INCOMING MAPS", geom_arrays.id_maps);
        // get lengths of existing entities before we start adding stuff
        // const num_posis: number = this._geom_arrays.num_posis;
        const num_posis: number = this._geom_arrays.up_posis_verts.length;
        const num_verts: number = this._geom_arrays.dn_verts_posis.length;
        ...
        // update the counts, must be the max of the two models
        // this means that there can never be a conflict in IDs
        for (const key of Object.keys(this._geom_arrays.id_counts)) {
            const a: number = this._geom_arrays.id_counts[key];
            const b: number = geom_arrays.id_counts[key];
            this._geom_arrays.id_counts[key] = a > b ? a : b;
        }
        ...
        for (let posi_i = 0; posi_i < geom_arrays.up_posis_verts.length; posi_i++) { // posis uses up array
            if (geom_arrays.up_posis_verts[posi_i] !== null) {
                const new_posi_i: number = posis_count + num_posis;
                posis_map.set(posi_i, new_posi_i);
                posis_count += 1;
                // update IDs for posis
                this._geom.id.mergeVertID(new_posi_i, geom_arrays.id_maps.posiID.get(posi_i));
            }
        }
        ...
        for (let vert_i = 0; vert_i < geom_arrays.dn_verts_posis.length; vert_i++) {
            const posi_i: number = geom_arrays.dn_verts_posis[vert_i];
            if (posi_i !== null) {
                const new_vert: TVert = posis_map.get(posi_i) as TVert;
                const new_vert_i: number = this._geom_arrays.dn_verts_posis.push( new_vert ) - 1;
                // update IDs
                this._geom.id.mergeVertID(new_vert_i, geom_arrays.id_maps.vertID.get(vert_i));
            }
        }
        ...
        for (const verts_i of geom_arrays.dn_tris_verts) {
            if (verts_i !== null) {
                ...
                // no need to add tris to ID maps
            }
        }
        // add edges to model
        for (let edge_i = 0; edge_i < geom_arrays.dn_edges_verts.length; edge_i++) {
            const verts_i: number[] = geom_arrays.dn_edges_verts[edge_i];
            if (verts_i !== null) {
                const new_edge: TEdge = verts_i.map(vert_i => verts_map.get(vert_i)) as TEdge;
                const new_edge_i: number = this._geom_arrays.dn_edges_verts.push( new_edge ) - 1;
                // update IDs
                this._geom.id.mergeVertID(new_edge_i, geom_arrays.id_maps.edgeID.get(edge_i));
            }
        }
        // add wires to model
        for (let wire_i = 0; wire_i < geom_arrays.dn_wires_edges.length; wire_i++) {
            const edges_i: number[] = geom_arrays.dn_wires_edges[wire_i];
            if (edges_i !== null) {
                const new_wire: TWire = edges_i.map(edge_i => edges_map.get(edge_i)) as TWire;
                const new_wire_i: number = this._geom_arrays.dn_wires_edges.push( new_wire ) - 1;
                // update IDs
                this._geom.id.mergeVertID(new_wire_i, geom_arrays.id_maps.wireID.get(wire_i));
            }
        }
        // add faces to model
        for (let face_i = 0; face_i < geom_arrays.dn_faces_wirestris.length; face_i++) {
            ...
            if (wires_tris_i !== null) {
                ...
                const new_face_i: number = this._geom_arrays.dn_faces_wirestris.push( new_face ) - 1;
                // update IDs
                this._geom.id.mergeVertID(new_face_i, geom_arrays.id_maps.faceID.get(face_i));
            }
        }
        // add points to model
        for (let point_i = 0; point_i < geom_arrays.dn_points_verts.length; point_i++) {
            const vert_i: number = geom_arrays.dn_points_verts[point_i];
            if (vert_i !== null) {
                const new_point: TPoint = verts_map.get(vert_i) as TPoint;
                const new_point_i: number = this._geom_arrays.dn_points_verts.push( new_point ) - 1;
                // update IDs
                this._geom.id.mergeVertID(new_point_i, geom_arrays.id_maps.pointID.get(point_i));
            }
        }
        // add plines to model
        for (let pline_i = 0; pline_i < geom_arrays.dn_plines_wires.length; pline_i++) {
            const wire_i: number = geom_arrays.dn_plines_wires[pline_i];
            if (wire_i !== null) {
                const new_pline: TPline = wires_map.get(wire_i) as TPline;
                const new_pline_i: number = this._geom_arrays.dn_plines_wires.push( new_pline ) - 1;
                // update IDs
                this._geom.id.mergeVertID(new_pline_i, geom_arrays.id_maps.plineID.get(pline_i));
            }
        }
        // add pgons to model
        for (let pgon_i = 0; pgon_i < geom_arrays.dn_pgons_faces.length; pgon_i++) {
            const face_i: number = geom_arrays.dn_pgons_faces[pgon_i];
            if (face_i !== null) {
                const new_pgon: TPgon = faces_map.get(face_i) as TPgon;
                const new_pgon_i: number = this._geom_arrays.dn_pgons_faces.push( new_pgon ) - 1;
                // update IDs
                this._geom.id.mergeVertID(new_pgon_i, geom_arrays.id_maps.pgonID.get(pgon_i));
            }
        }
        // add collections to model
        for (let coll_i = 0; coll_i < geom_arrays.dn_colls_objs.length; coll_i++) {
            const coll: TColl = geom_arrays.dn_colls_objs[coll_i];
            if (coll !== null) {
                ...
                const new_coll_i: number = this._geom_arrays.dn_colls_objs.push( new_coll ) - 1;
                // update IDs
                this._geom.id.mergeVertID(new_coll_i, geom_arrays.id_maps.collID.get(coll_i));
            }
        }
        ...
        console.log("RESULT", this._geom_arrays);
        // return teh maps
        return geom_maps;
    }
    // ======================================================================
    ...
    public setData(geom_data: IGeomData): IGeomPack {
        // set ID counts
        this._geom_arrays.id_counts = geom_data.id_counts;
        // update the ID maps
        this._geom_arrays.id_maps = this.convertIDArraysToMaps(geom_data.id_arrays);
    ...
    public getData(make_copy = false): IGeomData {
        const data: IGeomData = {
            id_counts: this._geom_arrays.id_counts,
            id_arrays: null, // this.convertIDMapsToArrays(),
        ...
        // set the ID arrays (must be arrays, so that they can be JSON.stringify())
        data.id_arrays = this.convertIDMapsToArrays();
        return data;
    }
    private convertIDMapsToArrays(): IIDArrays {
        const result: IIDArrays  = {
            posiID: null, posiIdx: null,
            vertID: null, vertIdx: null,
            edgeID: null, edgeIdx: null,
            wireID: null, wireIdx: null,
            faceID: null, faceIdx: null,
            pointID: null, pointIdx: null,
            plineID: null, plineIdx: null,
            pgonID: null, pgonIdx: null,
            collID: null, collIdx: null
        };
        for (const [key, value] of Object.entries(this._geom_arrays.id_maps)) {
            result[key] = Array.from(value);
        }
        return result;
    }
    private convertIDArraysToMaps(id_arrays: IIDArrays): IIDMaps {
        const result: IIDMaps  = {
            posiID: null, posiIdx: null,
            vertID: null, vertIdx: null,
            edgeID: null, edgeIdx: null,
            wireID: null, wireIdx: null,

import { EEntType, TEntTypeIdx } from './common';
...
// ============================================================================
// export function getIDList(model: GIModel, ent_type: EEntType, idxs: number|number[]|number[][]): TId|TId[]|TId[][] {
//     const depth: number = getArrDepth(idxs);       ++HAHALOL 
//     if (depth === 0) {
...
//         return idxs_arr.map( idx => __model__.geom.id.getID(model, ent_type, idx) ) as TId[];
//     } else { // depth === 2
//         const idxs_arrs: number[][] = idxs as number[][];
//         return idxs_arrs.map( idxs_arr => __model__.geom.id.getID(model, ent_type, idxs_arr) ) as TId[][];
//     }
// }
// export function getIDs(model: GIModel, ent_type_idxs: TEntTypeIdx|TEntTypeIdx[]|TEntTypeIdx[][]): TId|TId[]|TId[][] {
...
//         return model.geom.id.pgonID
//     } else if (depth === 2) {
//         const ent_type_idxs_arr: TEntTypeIdx[] = ent_type_idxs as TEntTypeIdx[];
//         return ent_type_idxs_arr.map( ent_type_idx => __model__.geom.id.getIDFromTypeIdx(model, ent_type_idx) ) as TId[];
//     } else { // depth === 3
//         const ent_type_idxs_arrs: TEntTypeIdx[][] = ent_type_idxs as TEntTypeIdx[][];
//         return ent_type_idxs_arrs.map( ent_type_idxs_arr => __model__.geom.id.getIDFromTypeIdx(model, ent_type_idxs_arr) ) as TId[][];
//     }
// }
// export function getIdxs(model: GIModel, ids: TId|TId[]|TId[][]): TEntTypeIdx|TEntTypeIdx[]|TEntTypeIdx[][] {
...
//         return ids_arr.map( id => getIdxsTOBERELACED(model, id) ) as TEntTypeIdx[];
//     } else { // depth === 2
//         const ids_arr: TId[][] = ids as TId[][];
//         return ids_arr.map( id => getIdxsTOBERELACED(model, id) ) as TEntTypeIdx[][];
//     }

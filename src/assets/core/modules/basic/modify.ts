...
export function Move(__model__: GIModel, entities: TId|TId[], vectors: Txyz|Txyz[]): void {
    ...
    if (!isEmptyArr(entities)) {
        ...
        // --- Error Check ---
        _move(__model__, ents_arrs, vectors);
    }
}
...
export function Rotate(__model__: GIModel, entities: TId|TId[], ray: Txyz|TRay|TPlane|TId|TId[], angle: number): void {
    ...
    if (!isEmptyArr(entities)) {
        ...
        // --- Error Check ---
        _rotate(__model__, ents_arrs, ray, angle);
    }
}
...
export function Scale(__model__: GIModel, entities: TId|TId[], plane: Txyz|TRay|TPlane|TId|TId[], scale: number|Txyz): void {
    ...
    if (!isEmptyArr(entities)) {
        ...
        // --- Error Check ---
        _scale(__model__, ents_arrs, plane, scale);
    }
}
...
export function Mirror(__model__: GIModel, entities: TId|TId[], plane: Txyz|TRay|TPlane|TId|TId[]): void {
    ...
    if (!isEmptyArr(entities)) {
        ...
        plane = getPlane(__model__, plane, fn_name) as TPlane;
        // --- Error Check ---
        _mirror(__model__, ents_arrs, plane);
    }
}
...
export function XForm(__model__: GIModel, entities: TId|TId[],
    ...
    if (!isEmptyArr(entities)) {
        ...
        // --- Error Check ---
        _xform(__model__, ents_arrs, from_plane, to_plane);
    }
}
...
export function Offset(__model__: GIModel, entities: TId|TId[], dist: number): void {
    ...
    if (!isEmptyArr(entities)) {
        ...
        // --- Error Check ---
        _offset(__model__, ents_arrs, dist);
    }
}
...
export function Reverse(__model__: GIModel, entities: TId|TId[]): void {
    ...
    if (!isEmptyArr(entities)) {
        // --- Error Check ---
        const ents_arrs = __model__.geom.id.getTypeIdxFromID(entities) as TEntTypeIdx[];
        checkIDs('modify.Reverse', 'entities', ents_arrs,
            [IDcheckObj.isID, IDcheckObj.isIDList],
            [EEntType.WIRE, EEntType.PLINE, EEntType.FACE, EEntType.PGON]);
        // --- Error Check ---
        _reverse(__model__, ents_arrs);
    }
}
...
export function Shift(__model__: GIModel, entities: TId|TId[], offset: number): void {
    ...
    if (!isEmptyArr(entities)) {
        ...
        checkIDs('modify.Reverse', 'entities', ents_arrs,
            ...
            [EEntType.WIRE, EEntType.PLINE, EEntType.FACE, EEntType.PGON]);
        // --- Error Check ---
        _shift(__model__, ents_arrs, offset);
    }
}
...
export function Ring(__model__: GIModel, entities: TId|TId[], method: _ERingMethod): void {
    ...
    if (!isEmptyArr(entities)) {
        ...
        const ents_arrs = __model__.geom.id.getTypeIdxFromID(entities) as TEntTypeIdx[];
        checkIDs(fn_name, 'entities', ents_arrs,
            [IDcheckObj.isID, IDcheckObj.isIDList], [EEntType.PLINE]);
        // --- Error Check ---
        _ring(__model__, ents_arrs, method);
    }
}
...
export function Weld(__model__: GIModel, entities: TId|TId[], method: _EWeldMethod): void {
    ...
    checkIDs(fn_name, 'entities', ents_arrs, [IDcheckObj.isID, IDcheckObj.isIDList], null);
    // --- Error Check ---
    _weld(__model__, ents_arrs, method);
}
...
export function Remesh(__model__: GIModel, entities: TId[]): void {
    ...
    if (!isEmptyArr(entities)) {
        // --- Error Check ---
        const ents_arrs = __model__.geom.id.getTypeIdxFromID(entities) as TEntTypeIdx[];
        checkIDs('modify.Remesh', 'entities', ents_arrs,
            [IDcheckObj.isID, IDcheckObj.isIDList], 
            [EEntType.FACE, EEntType.PGON, EEntType.COLL]);
        // --- Error Check ---
        _remesh(__model__, ents_arrs);
    }
}
    ...
    if (!isEmptyArr(entities)) {
        ...
        switch (method) {
            case _EDeleteMethod.DELETE_SELECTED:
                _delete(__model__, ents_arrs, false); //  do not invert
                break;
            case _EDeleteMethod.KEEP_SELECTED:
                _delete(__model__, ents_arrs, true); // invert
                break;
            default:
